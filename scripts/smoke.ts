...
${B}5в. Приватность: платёжные данные не хранятся${X}`);
  {
    ...
    check("в журнал не попадают телефон и email",
      !/701 234|test@mail/.test(R.redactForLog("звоните +7 701 234 56 78, test@mail.kz")));
...
${B}5г. Спецификация в Word; строки из PDF и фото${X}`);
  ...
  // ---- 5д. Поиск: кириллица, раскладка, синонимы ---------------------------
  // SQLite LOWER() понимает только латиницу: «узо» не находило «УЗО», и
  // выдача откатывалась к случайным клеммам. Эти проверки держат исправление.
  console.log(`\n${B}5д. Поиск по каталогу${X}`);
  {
    const first = async (q: string) => (await DB.searchCatalog(q))[0]?.name ?? "";
    check("«есть УЗО?» → УЗО", /УЗО|ВДТ/i.test(await first("есть УЗО?")), await first("есть УЗО?"));
    const auto = await first("автоматический выключатель на 16А");
    check("«автоматический выключатель на 16А» → автомат на 16 А, не бокс",
      /16\s?[АA]/.test(auto) && !/бокс/i.test(auto), auto);
    check("кириллица и латиница в артикуле — одно и то же",
      (await DB.getProduct("404029x"))?.sku === (await DB.getProduct("404029х"))?.sku
        && (await DB.getProduct("404029x")) !== null);
    check("«ВА 101 3Р» кириллицей находит «BA 101 3P»", /101 3[PР]/.test(await first("ВА 101 3Р")));
    check("выдуманный артикул не находится", (await DB.getProduct("нет-такого-999")) === null);
  }
